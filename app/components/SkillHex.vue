...
function onQuadrant() {
  switch (props.quadrant) {
    case 1:
      ...
      break
    case 2:
      imageClass.value = '-top-4 left-0 -right-2 -bottom-2'
      image.value = 'images/mobile.webp'
      skill.value = 'Mobile'
      break
    case 3:
...
onMounted(() => onQuadrant())
</script>

<template>
  <span class="relative drop-shadow-md transition-transform hover:scale-105">
    <NuxtIcon name="local:hexagon" class="size-[120px] fill-light-500 drop-shadow-md dark:fill-dark-600 lg:size-[180px]" />
    <NuxtImg v-if="image && skill" provider="ipx" :src="image" :alt="skill" :width="256" :height="256" loading="lazy" class="absolute drop-shadow-sm" :class="imageClass" />

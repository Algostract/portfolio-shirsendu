...
<template>
  <div
class="relative rounded-xl" :class="{
...
aria-label="right"
      ...
      <div class="absolute bottom-2 right-2 flex gap-2">
        <NuxtLink
aria-label="certificate" :to="`/static/certificates/${name}.pdf`" target="_blank"
        ...
        <NuxtLink
aria-label="link" :to="link" target="_blank"
          class="h-fit w-fit rounded-2xl bg-light-600 px-3 py-1 transition-colors hover:bg-primary-500 hover:text-white dark:bg-dark-600 dark:hover:bg-primary-500">
    ...
    <div class="aspect-[3/2] w-[18rem] overflow-hidden rounded-xl bg-light-500 dark:bg-dark-500 md:w-[28rem]">
      <NuxtImg
provider="uploadcare" :src="image?.id" :alt="image?.title" :width="512" :height="288" loading="lazy"
        class="size-full scale-100 object-cover transition-transform hover:scale-110" />
    </div>
  </div>
</template>

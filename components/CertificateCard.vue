  ...
  <div class="relative" :class="{ 'z-10 shadow-[0_4px_16px_4px_rgba(0,0,0,0.38)]': active, 'scale-90': !active }">
    <div v-if="active" class="absolute top-0 left-0 right-0 bottom-0 z-10">
      <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex justify-between w-[110%]">
        <button class="rounded-2xl px-3 py-2 w-fit h-fit bg-dark-600 hover:bg-primary-400 transition-colors"
          @click="emit('slide', 'left')">
          <NuxtIcon name="chevron-bold" class="text-[16px]" />
        </button>
        <button class="rounded-2xl px-3 py-2 w-fit h-fit bg-dark-600 hover:bg-primary-400 transition-colors rotate-180"
      ...
      <div class="absolute bottom-2 right-2 flex gap-2">
        <NuxtLink :href="`/certificates/${name}.pdf`" target="_blank"
          class="rounded-2xl px-3 py-1 w-fit h-fit bg-dark-600 hover:bg-primary-400 transition-colors">
